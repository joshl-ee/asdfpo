                ...
                const Item = await dynamoDbClient.scan(getParams).promise().then(
                    getData => {
                        if (getData.Count == 0) {
                            ...
                            unique = true;
                            debuglog("SetExperiment: Instance: " + JSON.stringify(instance));
                            var instanceNoJWTToken = instance;
                            delete instanceNoJWTToken.jwtToken;
                            debuglog("SetExperiment: instanceNoJWTToken " + JSON.stringify(instanceNoJWTToken));

                            putParams = {
                                TableName: DATA_TABLE,
                                Item: instanceNoJWTToken,
                            };
                        }
                    }
                );
